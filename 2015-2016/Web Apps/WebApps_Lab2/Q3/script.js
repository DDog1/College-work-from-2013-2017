...
function makeRequest() {
	...
	// Uses the GET command to get the colors.xml file, true is for asynchronous
		xhr.open("GET", "http://localhost:8080/WebApps/WebApps_Lab2/Q3/Q1_Part_B.xml", true);
	// Sends the request
		xhr.send(null);
	}
	else {
	// put an error message into the body with id "updateArea" on HTML document
		document.getElementById("update").innerHTML = "Sorry, but I couldn't create an XMLHttpRequest";
	}
}
...
// if readystate = 4 => complete
	...
	// if status = 200 => ok
		...
		document.getElementById("update").innerHTML = outMsg;
	}
}
